      ...
      <div className="flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800/80">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="p-1 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 sm:hidden cursor-pointer"
            aria-label="Close"
          >
            <X size={15} />
          </button>
  ...
  return (
    <>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        {/* Desktop Popover Content (sm and up) */}
        <PopoverContent
          align="end"
          sideOffset={8}
          className="hidden sm:block z-50 w-[min(23rem,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-zinc-200/90 bg-white p-4 shadow-2xl dark:border-zinc-800 dark:bg-[#121215]"
        >
          {plannerContent}
        </PopoverContent>
      </Popover>

      {/* Mobile Bottom Island Options Sheet rendered via React Portal directly to document.body */}
      {mounted &&
        open &&
        createPortal(
