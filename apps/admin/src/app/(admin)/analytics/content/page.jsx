...
export default function ContentPage() {
  ...
  useEffect(() => {
    analyticsApi
      .ga4(controls.range)
      .then((r) => (r.success ? setData(r.data.data) : setError(r.error)));
  }, [controls.range]);
  let pages = data?.pages || [];
  if (tab === "Engagement")
    pages = [...pages]
      .filter((p) => p.screenPageViews >= 5)
      .sort((a, b) => b.averageEngagementTime - a.averageEngagementTime);
  if (tab === "Needs improvement")
    pages = [...pages]
      .filter((p) => p.screenPageViews >= 20)
      .sort((a, b) => a.averageEngagementTime - b.averageEngagementTime);
  return (
    <AnalyticsShell
      eyebrow="GA4 · Author workspace"
      title="Content performance"
      description="Which pages attract visitors and hold their attention. Content types are not guessed from ambiguous URLs."
      actions={
        <RangeControls
          {...controls}
    ...
    >
      <div className="inline-flex max-w-full items-center gap-1 p-1 overflow-x-auto">
        {["Top pages", "Landing pages", "Engagement", "Needs improvement"].map(
          (name) => (
            <button
              key={name}
              onClick={() => setTab(name)}
      ...
      {error && (
        <p className="rounded-2xl bg-amber-50 p-4 text-amber-800">{error}</p>
      )}
      {tab === "Landing pages" ? (
        <Table
          headers={["Landing page", "Sessions", "Users", "Engagement"]}
        ...
        <Table
          headers={["Page", "Title", "Views", "Users", "Avg engagement"]}
          rows={pages.map((r) => [
            r.pagePath,
            r.pageTitle || "Untitled",
            n(r.screenPageViews),
            n(r.activeUsers),
            `${Math.round(r.averageEngagementTime)}s`,
          ])}
        />
      )}
    </AnalyticsShell>
