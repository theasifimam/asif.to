    ...
    audienceType === "countries"
      ? [
          ...
          },
          {
            key: "screenPageViews",
            label: "Page views",
            render: (row) => n(row.screenPageViews),
          },
        ]
      : [
          {
            ...
            render: (row) => (
              <strong>
                {row[audienceType === "devices" ? "device" : "browser"] ||
                  "(not set)"}
              </strong>
            ),
          },
    ...
    <div className="space-y-10">
      ...
      <section className="grid grid-cols-2 gap-1 lg:grid-cols-4">
        ...
        <MetricCard
          icon={Eye}
          label="Page views"
          value={n(summary.screenPageViews)}
          delta={deltas.screenPageViews}
          source="GA4"
        />
        <MetricCard
          icon={Activity}
          label="Engaged sessions"
          value={n(summary.engagedSessions)}
          delta={deltas.engagedSessions}
          source="GA4"
        />
        <MetricCard
          icon={BarChart3}
          label="Engagement rate"
          value={ratioPct(summary.engagementRate)}
        ...
        <MetricCard
          ...
          value={duration(summary.userEngagementDuration)}
          delta={deltas.userEngagementDuration}
          source="GA4"
        />
        <MetricCard
          icon={Clock3}
          label="Avg. engagement / active user"
          value={seconds(summary.averageEngagementTime)}
          source="GA4"
        />
      </section>

      {/* 2-column: GA4 Traffic Trend (2/3 space) + GA4 Top Events (1/3 space) */}
      <div className="grid gap-1 xl:grid-cols-[1.3fr_0.7fr]">
        <Section
          eyebrow="GA4 trend"
          title="GA4 traffic over time"
          description="Active users, sessions and page views from the GA4 Data API."
        >
          <TrendChart
            data={data?.trend || []}
            series={[
        ...
        <Section
          eyebrow="Events"
          title="GA4 events"
          description="Key actions triggered by users."
        >
          <ClientTable
            rows={data?.events || []}
            pageSize={6}
            columns={[
              {
                ...
                label: "Event",
                render: (row) => (
                  <strong>{row.eventName || "(not set)"}</strong>
                ),
              },
              {
                key: "eventCount",
                label: "Count",
                render: (row) => n(row.eventCount),
              },
              {
                key: "totalUsers",
                label: "Users",
                render: (row) => n(row.totalUsers),
              },
            ]}
          />
        </Section>
      ...
      <Section
        ...
        description="All page fields currently returned by your GA4 integration."
      >
        <ClientTable
          rows={data?.pages || []}
          columns={[
            {
              ...
              render: (row) => (
                <a
                  href={`https://asif.to${row.pagePath}`}
                  target="_blank"
                  rel="noreferrer"
                  className="block max-w-72 truncate font-black text-blue-600 hover:underline"
                >
                  {row.pagePath}
                </a>
              ),
            ...
            },
            {
              ...
              label: "Total engagement",
              render: (row) => duration(row.userEngagementDuration),
            },
            {
              key: "averageEngagementTime",
              label: "Avg. engagement / view",
              render: (row) => seconds(row.averageEngagementTime),
            },
          ]}
        />
      </Section>
      ...
      <div className="grid gap-1 xl:grid-cols-2">
        <Section
          ...
          description="Sessions, users and engagement where sessions began."
        >
          <ClientTable
            ...
            columns={[
              {
                ...
                label: "Landing page",
                render: (row) => (
                  <strong>{row.landingPage || "(not set)"}</strong>
                ),
              },
              {
                key: "sessions",
              ...
              },
              {
                key: "engagedSessions",
                label: "Engaged",
                render: (row) => n(row.engagedSessions),
              },
              {
                key: "engagementRate",
                label: "Eng. rate",
                render: (row) => ratioPct(row.engagementRate),
              },
            ]}
          />
        </Section>
        ...
        <Section
          ...
          description="Channel group and source / medium metrics."
        >
          <ClientTable
            ...
            columns={[
              {
                key: "channel",
                label: "Channel",
                render: (row) => <strong>{row.channel || "(not set)"}</strong>,
              },
              {
                key: "sourceMedium",
                label: "Source / medium",
      ...
      >
        ...
        <div className="grid gap-1 xl:grid-cols-[220px_1fr]">
          <MetricCard
            icon={Radio}
            label="Active users now"
            value={n(realtime?.activeUsers)}
            source="GA4 realtime"
          />
          <ClientTable
            rows={realtime?.rows || []}
