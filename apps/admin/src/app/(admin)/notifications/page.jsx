...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-4xl p-5 sm:p-8"><div className="mb-6 flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-blue-600">Inbox</p><h1 className="text-3xl font-black tracking-tight dark:text-white">Notifications</h1></div><button onClick={allRead} className="flex items-center gap-2 text-xs font-bold text-blue-600"><CheckCheck size={15}/> Mark all read</button></div><div className="mb-4 inline-flex max-w-full items-center gap-1 p-1 overflow-x-auto">{["all", "unread", "important", "critical"].map((value) => <button key={value} onClick={() => setTab(value)} className={`rounded-full px-4 py-2 text-xs font-bold capitalize transition-all cursor-pointer ${tab === value ? "bg-white text-blue-600 shadow-xs dark:bg-zinc-800 dark:text-blue-400" : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"}`}>{value}</button>)}</div><div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-[#121215]">{data.notifications.map((item) => <Link key={item._id} href={item.url || "/activity"} onClick={() => !item.isRead && activityApi.markRead(item._id)} className={`block border-b border-zinc-100 p-4 last:border-0 dark:border-zinc-800 ${!item.isRead ? "bg-blue-50/60 dark:bg-blue-500/5" : ""}`}><p className="text-sm font-bold dark:text-zinc-100">{item.message}</p><p className="mt-1 text-[11px] text-zinc-500">{item.activityId?.actorId?.fullName || "System"} · {new Date(item.createdAt).toLocaleString()}</p></Link>)}{!data.notifications.length && <p className="p-10 text-center text-sm text-zinc-500">No notifications here.</p>}</div></div>;
}
