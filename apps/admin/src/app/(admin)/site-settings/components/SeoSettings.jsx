...
export default function SeoSettings() {
  ...
  useEffect(() => {
    seoSettingsApi.list().then((response) => {
  ...
  const choose = (path) => {
    ...
    setForm(item ? { ...item, keywords: (item.keywords || []).join(", ") } : { ...empty, path });
  };

  const save = async () => {
    setSaving(true);
    const response = await seoSettingsApi.save({
      ...form,
    ...
    setItems((current) => [...current.filter((item) => item.path !== saved.path), saved].sort((a, b) => a.path.localeCompare(b.path)));
    toast.success("SEO settings saved");
  };

  return (
    <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)] min-w-0 w-full mt-0">
      <aside className="admin-surface rounded-3xl p-4 self-start min-w-0 w-full bg-white dark:bg-[#121215] shadow-xs border border-zinc-200/80 dark:border-zinc-800/80">
      ...
      <main className="admin-surface space-y-5 rounded-3xl p-6 sm:p-8 min-w-0 w-full bg-white dark:bg-[#121215] shadow-xs border border-zinc-200/80 dark:border-zinc-800/80">
