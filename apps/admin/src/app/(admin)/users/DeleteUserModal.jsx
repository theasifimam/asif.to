...
export function DeleteUserModal({ user, onClose, onDelete, submitting }) {
  ...
  return (
    <Dialog open={!!user} onOpenChange={(o) => {if (!o) { setReason(""); onClose(); }}}>
      <DialogContent variant="island" className="p-6 sm:p-8 sm:max-w-[440px] rounded-[2rem]">
        <DialogHeader className="items-center text-center gap-3 sm:gap-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-[24px] bg-red-50 dark:bg-red-500/10 flex items-center justify-center text-red-500">
            <Trash2 size={28} />
          </div>
          <DialogTitle className="text-xl sm:text-2xl font-black font-outfit uppercase tracking-tighter text-zinc-900 dark:text-white">Delete User?</DialogTitle>
        ...
        <DialogFooter className="mt-6 grid grid-cols-2 gap-2.5 sm:flex sm:flex-row sm:gap-3">
          <Button variant="ghost" onClick={onClose} className="w-full sm:flex-1 h-11 sm:h-12 px-4 sm:px-6 rounded-full text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-zinc-900 dark:hover:text-white border border-zinc-200 dark:border-zinc-800 transition-colors cursor-pointer">Cancel</Button>
          <Button
