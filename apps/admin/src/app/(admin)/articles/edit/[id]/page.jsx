...
import { articlesApi, articleTopicsApi, Topic } from "@/lib/api";
...
export default function EditArticlePage({ params: paramsPromise }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch topics
        const topicsRes = await articleTopicsApi.list();
        ...
        if (articleRes.success && articleRes.data) {
          ...
          setSelectedTopics(
            article.topic.map((t) => (typeof t === "string" ? t : t._id || t)),
          );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || "Failed to update signal", {
          id: toastId,
        });
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-[#09090b] flex items-center justify-center p-12">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[11px] font-black uppercase tracking-[0.6em] text-zinc-500 animate-pulse">
            Syncing Archive...
          </span>
        </div>
      </div>
      ...
      <header className="h-20 border-b border-zinc-200 dark:border-zinc-900 px-12 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl z-[120] transition-colors duration-300">
        <div className="flex items-center gap-8">
          <Link
            ...
            className="w-10 h-10 rounded-full border border-zinc-200 dark:border-zinc-800 flex items-center justify-center hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all text-zinc-500 hover:text-zinc-900 dark:hover:text-white bg-white dark:bg-transparent shadow-sm dark:shadow-none"
          >
            <ChevronLeft size={20} />
          </Link>
          <div className="flex flex-col">
            ...
            <span className="text-sm font-black text-zinc-900 dark:text-white italic tracking-tight">
              {title || "EDITING SIGNAL..."}
            </span>
          </div>
        </div>
          ...
          <div className="max-w-4xl mx-auto flex flex-col gap-12">
            ...
            <div className="w-full">
              <Editor
                ...
                placeholder="RE-WRITE DISPATCH PROSE..."
              />
            </div>
          </div>
        </main>
            ...
            >
              <div className="max-w-[1400px] mx-auto bg-white dark:bg-[#09090b] text-zinc-900 dark:text-white shadow-2xl shadow-black/5 dark:shadow-none rounded-[3rem] p-12 md:p-32 min-h-[80vh] border border-zinc-100 dark:border-zinc-900 transition-colors duration-500">
                <div className="flex items-center gap-4 mb-20 text-zinc-900 dark:text-white">
                  ...
                  <span className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-500 dark:text-zinc-600">
                    Re-Transmitting Projection
                  </span>
                </div>
                    ...
                    {selectedTopics.map((topicId) => {
                      ...
                      return (
                        <Badge
                          key={topicId}
                          variant="secondary"
                          className="px-4 py-2 bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-zinc-200 dark:border-transparent transition-colors duration-300"
                        >
                          {topic?.name || "Topic"}
                        </Badge>
                      );
                ...
                <div
                  className={`aspect-[21/9] ${heroImage ? "bg-transparent border-none shadow-xl shadow-black/5 dark:shadow-none" : "bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-transparent"} rounded-[2.5rem] mb-20 overflow-hidden relative group transition-colors duration-300`}
                >
                  {heroImage ? (
                    <img
                      src={heroImage}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-zinc-300 dark:text-zinc-800 font-black tracking-widest text-xl uppercase italic">
                      No Visual Loaded
                    </div>
                  )}
                </div>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      content ||
                      '<p class="text-zinc-300 dark:text-zinc-800 italic font-light animate-pulse">[ Dispatch Prose Rendering in Real-time ]</p>',
                  }}
                />
              </div>
            </motion.div>
              ...
              {availableTopics.map((topic) => {
                ...
                return (
                  <Badge
                    ...
                    className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${
                      isSelected
