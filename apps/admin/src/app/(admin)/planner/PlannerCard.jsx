...
export default function PlannerCard({ card, onOpen, overlay = false }) {
  ...
  return (
    <article ref={sortable.setNodeRef} style={style} onClick={() => onOpen?.(card)} className={`group cursor-pointer rounded-2xl border border-zinc-200/80 bg-white p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950 dark:hover:border-blue-800 ${overlay ? "w-72 rotate-2 shadow-xl" : ""}`}>
      <div className="flex items-start gap-2">
        <button {...sortable.attributes} {...sortable.listeners} onClick={(event) => event.stopPropagation()} className="-ml-1 mt-0.5 cursor-grab touch-none text-zinc-300 opacity-0 transition group-hover:opacity-100 dark:text-zinc-700" aria-label="Drag card"><GripVertical size={15} /></button>
        <div className="min-w-0 flex-1">
          ...
          <h3 className="text-sm font-bold leading-snug text-zinc-900 dark:text-zinc-100">{card.title}</h3>
          {card.labels?.length > 0 && <div className="mt-2.5 flex flex-wrap gap-1">{card.labels.slice(0, 3).map((label) => <span key={label._id} className="rounded-full px-2 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: `${label.color}20`, color: label.color }}>{label.name}</span>)}</div>}
          {(card.dueDate || total > 0 || card.parentCard) && <div className="mt-3 flex items-center gap-3 text-[11px] font-semibold text-zinc-500">
