...
export const SEARCH_TYPES = { all: "All", user: "Users", job: "Jobs", company: "Companies", "interview-category": "Interview categories", course: "Courses", chapter: "Chapters", topic: "Topics", article: "Articles", question: "Questions", cheatsheet: "Cheatsheets" };
...
const matches = (needle, haystack) => haystack.includes(needle) || haystack.some((word) => needle.length >= 2 && word.startsWith(needle)) || haystack.some((word) => needle.length > 3 && Math.abs(word.length - needle.length) <= 2 && distance(needle, word) <= (needle.length > 7 ? 2 : 1));
