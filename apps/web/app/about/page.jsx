    ...
    return (
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 flex flex-col items-center justify-center">
          ...
          <span className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-6 md:px-12 pt-24 md:pt-28 pb-20">
        ...
        >
          ...
          {page?.content ? (
            ...
            <p className="text-xl md:text-2xl font-light leading-relaxed text-zinc-500 dark:text-zinc-400 italic max-w-3xl">
              "asif News was founded on a single premise: that the
              infrastructure of our information determines the quality of our
              reality."
            </p>
          )}
        </motion.section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-8 my-20 pt-12 border-t border-zinc-200/60 dark:border-zinc-800/60">
          {[
            ...
            <div
              key={value.title}
              className="flex flex-col gap-4 p-8 bg-white dark:bg-[#121215] rounded-3xl border border-zinc-200/80 dark:border-zinc-800/80 shadow-sm hover:shadow-md transition-all"
            >
              <div className="w-10 h-10 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
                {value.icon}
              </div>
              <h3 className="text-lg font-bold font-outfit tracking-tight text-zinc-900 dark:text-white uppercase">
                {value.title}
              </h3>
