...
export async function generateMetadata({ params }) {
  ...
  if (username.toLowerCase() === "asif")
    return {
      title: "Asif Imam — Full-Stack JavaScript Developer and Author",
      ...
      openGraph: {
        title: "Asif Imam— Author and Developer behind asif.to",
        description: authorIdentity.shortBio,
...
export default async function AuthorProfilePage({ params }) {
  ...
  if (username.toLowerCase() === "asif") {
    ...
    const [profileBody, coursesBody, articlesBody] = await Promise.all([
      safeFetch("/users/public/asif"),
      safeFetch("/courses"),
      safeFetch("/articles?status=published&limit=12"),
    ]);
    ...
    const schema = {
      ...
      "@graph": [
        buildPersonSchema({
          image: profile?.avatar ? assetUrl(profile.avatar) : undefined,
        }),
        {
          "@type": "ProfilePage",
          "@id": `${authorIdentity.url}#profile`,
          url: authorIdentity.url,
          name: "Asif — Full-Stack JavaScript Developer",
          mainEntity: { "@id": `${authorIdentity.url}#person` },
          isPartOf: {
            "@type": "WebSite",
            name: "asif.to",
            url: absoluteUrl("", "/"),
