 ...
 */
export default function ExamTimer({ onExpire, startedAt }) {
  const [secondsLeft, setSecondsLeft] = useState(() => {
    // Resume from sessionStorage if available
    const saved = sessionStorage.getItem("exam_timer_start");
    if (saved) {
  ...
  useEffect(() => {
    // Persist timer start
    if (!sessionStorage.getItem("exam_timer_start")) {
      sessionStorage.setItem("exam_timer_start", Date.now().toString());
    }
  ...
  return (
    <div className={`flex items-center gap-2.5 px-3 py-2 rounded-2xl ${bgColor} transition-colors duration-1000`}>
      <div className="relative w-10 h-10 shrink-0">
        ...
        <Clock
          className={`absolute inset-0 m-auto w-4 h-4 ${textColor}`}
        />
      </div>

      <div className="flex flex-col leading-none">
        <span className={`text-xs font-extrabold tabular-nums ${textColor} transition-colors`}>
          {String(minutes).padStart(2, "0")}:{String(seconds).padStart(2, "0")}
        </span>
        <span className="text-[10px] text-zinc-400 font-bold mt-0.5">remaining</span>
      </div>
    </div>
  );
}
