...
import { Loader2, Maximize2, Minimize2, Play, RotateCcw, Trash2 } from "lucide-react";
...
export default function BrowserRuntimeEditor({ language, languageOptions, onLanguageChange, code, files, title, playgroundId = "scratch", fillViewport = false }) {
  ...
  useEffect(() => {
    const onFullscreenChange = () => setFullscreen(document.fullscreenElement === workspaceRef.current);
  ...
  return <section ref={workspaceRef} className={`overflow-hidden border border-zinc-800 bg-[#181818] text-white shadow-2xl ${fullscreen ? "fixed inset-0 z-100 h-screen max-h-none rounded-none" : `rounded-2xl sm:rounded-3xl ${fillViewport ? "h-[calc(100dvh-130px)] min-h-120" : "h-[68vh] min-h-115 max-h-175"}`}`} aria-label={`${config.label} browser code editor`}>
    <header className="flex h-12 items-center justify-between gap-3 border-b border-zinc-800 px-3 sm:px-4"><div className="flex min-w-0 items-center gap-2"><strong className="truncate text-sm">{title || `${config.label} Playground`}</strong>{languageOptions && onLanguageChange && <select value={language} onChange={(event) => onLanguageChange(event.target.value)} className="max-w-36 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" aria-label="Select language">{languageOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>}</div><div className="flex items-center gap-2"><span className="hidden text-[11px] text-zinc-500 sm:inline" aria-live="polite">{saveStatus}</span><button type="button" onClick={toggleFullscreen} className="rounded-lg bg-zinc-800 p-2 text-zinc-300 hover:bg-zinc-700" aria-label={fullscreen ? "Exit fullscreen editor" : "Open fullscreen editor"} title={fullscreen ? "Exit fullscreen" : "Open fullscreen"}>{fullscreen ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button><button type="button" onClick={() => { setSource(starter); setOutput(""); setError(""); }} className="rounded-lg bg-zinc-800 p-2 text-zinc-300 hover:bg-zinc-700" aria-label="Reset code" title="Reset code"><RotateCcw className="h-4 w-4" /></button><button type="button" onClick={run} disabled={status === "loading"} className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black hover:bg-emerald-500 disabled:cursor-wait disabled:opacity-60" title="Run (Ctrl/Cmd + Enter)">{status === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4 fill-current" />}Run</button></div></header>
