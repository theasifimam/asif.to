...
function codeLanguage(codeElement) {
  ...
  return languageClass?.slice("language-".length).replace(/-(?:play|no-play|static)$/, "") || "javascript";
}

function playgroundDirective(pre) {
  const code = pre.querySelector("code");
  const metadata = [pre.dataset.meta, code?.dataset?.meta, pre.className, code?.className]
    .filter(Boolean)
    .join(" ");
  if (/(?:^|\s)(?:no-play|static)(?:\s|$)/i.test(metadata) || /-(?:no-play|static)(?:\s|$)/i.test(metadata)) return false;
  if (/(?:^|\s)play(?:\s|$)/i.test(metadata) || /-play(?:\s|$)/i.test(metadata)) return true;
  let sibling = pre.previousSibling;
  for (let index = 0; sibling && index < 4; index += 1, sibling = sibling.previousSibling) {
    if (sibling.nodeType === Node.COMMENT_NODE && /asif\s*:\s*no-play/i.test(sibling.textContent || "")) return false;
    if (sibling.nodeType === Node.COMMENT_NODE && /asif\s*:\s*play/i.test(sibling.textContent || "")) return true;
...
export default function MarkdownCodePlayground({
  ...
  const addPlayButtons = useCallback(() => {
    rootRef.current?.querySelectorAll("pre").forEach((pre) => {
      ...
      button.className = "absolute right-2 top-2 z-10 inline-flex items-center gap-1.5 rounded-full bg-emerald-500 px-3 py-1.5 text-xs font-black text-white shadow-lg transition hover:bg-emerald-400 active:scale-95";
      button.setAttribute("aria-label", "Open this code in the interactive editor");
  ...
  const handleClick = (event) => {
    ...
    setSnippet({ code, language: codeLanguage(codeElement), title: "Code example" });
  };
