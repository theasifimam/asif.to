      ...
      >
        {!logs.length ? (
          ...
          logs.map((log) => (
            <pre
              key={log.id}
              className={`m-0 w-full whitespace-pre-wrap wrap-break-word border-b border-zinc-800/70 py-2 last:border-0 ${log.method === "error" ? "text-red-400" : log.method === "warn" ? "text-amber-400" : log.method === "info" ? "text-blue-300" : "text-zinc-200"}`}
            >
              {(log.data || []).map(printable).join(" ")}
            </pre>
          ))
        )}
      </div>
