...
export default function CheatsheetReader({ slug, initialData }) {
  ...
  if (isLoading && !initialData) return <div className="flex min-h-screen items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-blue-500" /></div>;
  ...
  return <div className="min-h-screen bg-zinc-50 text-foreground dark:bg-zinc-950"><Header /><main className="mx-auto w-full max-w-4xl px-4 pb-24 pt-24 sm:px-6">
    ...
    <article className="mt-6 rounded-4xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-10"><TopicMarkdown content={cheatsheet.content} /></article>
  </main><Footer /></div>;
}
