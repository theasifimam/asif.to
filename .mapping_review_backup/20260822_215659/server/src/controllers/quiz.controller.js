...
export const getQuizQuestionsAdmin = async (req, res) => {
  try {
    ...
    if (courseId && mongoose.isValidObjectId(courseId)) {
      if (type === "all")
        filter.$or = [{ courses: courseId }, { course: courseId }];
      else if (type === "interview") filter.course = courseId;
      else filter.courses = courseId;
    }
    if (type !== "interview" && (chapterId || categoryId)) {
      const elem = {};
      if (courseId && mongoose.isValidObjectId(courseId))
    ...
    if (type !== "interview" && mapping === "mapped")
      filter["learningMappings.0"] = { $exists: true };
    if (type !== "interview" && mapping === "unmapped")
      filter["learningMappings.0"] = { $exists: false };
