...
export default function QuestionsPage() {
  ...
  const [filters, setFilters] = useUrlFilters({
    courseId: "all",
    type: "all",
    // ASIF_QUESTION_LEARNING_MAPPING_V1:list-state
    mapping: "all",
    search: "",
    view: "table",
  });
  const { courseId, type, mapping, search } = filters;
  const setMapping = (value) => setFilters((current) => ({ ...current, mapping: value }));
...
{/* ASIF_QUESTION_LEARNING_MAPPING_V1:filter-ui */}
<div className="w-full md:w-44">
  <Select value={mapping} onValueChange={filter(setMapping)}>
    <SelectTrigger className="h-10 w-full rounded-full border border-zinc-200/80 bg-white/90 px-4 text-xs font-semibold dark:border-zinc-800/80 dark:bg-[#18181b]"><SelectValue placeholder="Learning mapping" /></SelectTrigger>
    <SelectContent><SelectItem value="all">All mappings</SelectItem><SelectItem value="mapped">Mapped</SelectItem><SelectItem value="unmapped">Unmapped</SelectItem></SelectContent>
  </Select>
</div>
