...
export const updateCollection = async (req, res) => { const existing = await LibraryCollection.findOne({ _id: req.params.id, userId: req.user._id }); if (!existing) return res.status(404).json({ success: false, message: "Collection not found." }); if (visibilityOf(req.body.visibility) === "public" && existing.visibility !== "public" && req.body.publishConfirmed !== true) return res.status(400).json({ success: false, message: "Confirm the public publishing warning before continuing." }); const item = await LibraryCollection.findByIdAndUpdate(existing._id, { $set: { ...(req.body.name !== undefined ? { name: String(req.body.name).slice(0, 100) } : {}), ...(req.body.description !== undefined ? { description: String(req.body.description).slice(0, 500) } : {}), ...(req.body.visibility !== undefined ? { visibility: visibilityOf(req.body.visibility) } : {}) } }, { returnDocument: 'after', runValidators: true }); res.json({ success: true, data: item }); };
