...
export async function getSearchIndex(_req, res) {
  try {
    ...
    const [courses, chapters, topics, articles, questions] = await Promise.all([
      Course.find({ status: "published" }).select("title slug subtitle keywords techId order").lean(),
      Chapter.find({ status: "published" }).select("title slug summary keywords content course order").populate("course", "title slug techId").lean(),
      CourseTopic.find({ status: "published" }).select("title slug excerpt keywords content type course category order").populate("course", "title slug techId").populate("category", "name slug").lean(),
      Article.find({ status: "published" }).select("title slug content seoDescription keywords type techId order topic").populate("topic", "name").lean(),
      Question.find({ type: "interview" }).select("question slug answer tags difficulty course").populate("course", "title slug techId status").lean(),
    ]);
