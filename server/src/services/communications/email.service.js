...
export function brandedEmail({ text = "", stream = "TRANSACTIONAL", unsubscribeUrl = "" }) {
  ...
  const unsubscribe = unsubscribeUrl && ["MARKETING", "JOBS"].includes(stream)
    ? `<p style="margin:24px 0 0;font-size:12px;line-height:1.6;color:#71717a">You are receiving this because you subscribed to asif.to updates. <a href="${escapeHtml(unsubscribeUrl)}" style="color:#2563eb">Unsubscribe or manage preferences</a>.</p>`
    : "";
  return `<!doctype html><html><body style="margin:0;background:#f4f4f5;color:#18181b;font-family:Arial,Helvetica,sans-serif"><div style="padding:32px 16px"><div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #e4e4e7;border-radius:24px;overflow:hidden"><div style="padding:24px 28px;background:linear-gradient(135deg,#eff6ff,#eef2ff);border-bottom:1px solid #dbeafe"><a href="${escapeHtml(siteUrl)}" style="text-decoration:none;color:#18181b"><img src="${escapeHtml(logoUrl)}" width="40" height="40" alt="asif.to" style="display:block;border-radius:12px;margin-bottom:12px" /><strong style="font-size:22px;letter-spacing:-.04em">asif<span style="color:#2563eb">.to</span></strong></a></div><div style="padding:32px 28px;font-size:15px;line-height:1.7">${content}${unsubscribe}</div><div style="padding:20px 28px;background:#fafafa;border-top:1px solid #f4f4f5;color:#71717a;font-size:12px;line-height:1.6"><strong style="color:#3f3f46">asif.to</strong><br />Coding tutorials, courses and developer resources.<br /><a href="${escapeHtml(siteUrl)}" style="color:#2563eb">Visit asif.to</a> · <a href="${escapeHtml(siteUrl)}/contact" style="color:#2563eb">Contact support</a></div></div></div></body></html>`;
}
...
export async function sendLegacyEmail(mail, { stream = "TRANSACTIONAL" } = {}) {
  ...
  try {
    const result = await (stream === "SUPPORT" ? getSupportTransporter() : getTransporter()).sendMail({ ...mail, html: brandedEmail({ text: mail.text, stream }) });
...
export async function deliverJob(job, config, provider) {
  ...
  const result = await transport.sendMail({
    ...
    replyTo,
    subject: job.mail.subject, text, html: brandedEmail({ text, stream: job.stream, unsubscribeUrl: marketing ? url : "" }),
