...
const TARGET_COURSE = args[0] || "javascript";

const STOP = new Set(
  "a an and are as at be been but by can do does for from how if in into is it of on or should that the this to use used using what when where which why with you your javascript js course chapter question answer example".split(" ")
);
...
async function main() {
  await connectDB();
  console.log("\n=======================================================");
  console.log(`Categorizing Quiz Questions for Target: "${TARGET_COURSE}"`);
  console.log("=======================================================\n");

  const course = await Course.findOne({
    $or: [
      { slug: TARGET_COURSE },
      { techId: TARGET_COURSE },
      { title: new RegExp(TARGET_COURSE.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") },
    ],
  }).sort({ createdAt: 1 });

  if (!course) {
    console.error(`Course matching "${TARGET_COURSE}" not found!`);
    process.exit(1);
  }
  ...
  // Fetch only QUIZ questions (excluding interview questions)
  ...
  console.log(
    `Found ${questions.length} quiz questions to categorize for ${course.title}.\n`
  );
  ...
  for (const q of questions) {
    ...
    // First check tag pattern if seeded via tag e.g. "prefix:chapter-slug:..."
