...
const currentUri =
  process.env.MONGO_URI || "mongodb://127.0.0.1:27017/news-asif";
...
async function backup() {
  ...
  try {
    ...
    // Extract db name from URI or default to news-asif
    const dbName = currentUri.split("/").pop().split("?")[0] || "news-asif";
    ...
    for (const col of collections) {
      ...
      console.log(
        `Exported ${docs.length} documents from collection '${col.name}' to ${filePath}`,
      );
    }
